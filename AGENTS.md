...
> Repository development instructions for **database** (Shared Package).
> Stack: python-package. Generated as part of the Omnixys repository-policy rollout.

## Repository Overview

Omnixys shared database package (SQLAlchemy async).

- Repository path: `packages/python/database` (relative to the Omnixys root)
...
src/database/ SQLAlchemy async base, session, helpers
...
Persistence helpers shared across Python services; SQLAlchemy mypy plugin.
