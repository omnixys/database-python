<!-- repository: packages/python/database | kind: PACKAGE | stack: python-package -->

# database — Skill: Package Development

> Workflow for database (packages/python/database). Execute this workflow before, during, and
> after changes in this repository.

## Repository Facts

- Kind: Shared Package
- Package: `omnixys-database` (version: dynamic)
- Runtime: Python >=3.14 (uv)
- Description: Omnixys shared database package (SQLAlchemy async).
- Architecture: src/database/ SQLAlchemy async base, session, helpers
...
- This package is published (`omnixys-database`); consumers pin versions and rely on SemVer.
